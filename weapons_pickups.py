...
def weapons_pickups():
	...
	while break_queue == False:
		...
		elif ('matches' not in player_con) and (user_prompt.lower() != '[stop]'):
		...
		elif ('matches' not in player_con) and (user_prompt.lower() == '[stop]') and (break_prompt.lower() == 'y'):
			print('Process has been terminated.  ')
			print()
		# inform the user that the process has resumed
		# if they enter "[STOP]" and press N
		elif ('matches' not in player_con) and (user_prompt.lower() == '[stop]') and (break_prompt.lower() == 'n'):
			print('Resuming process.  ')
			print()
		# inform the user that they haven't pressed Y or N
		# after entering [STOP]
		elif ('matches' not in player_con) and (user_prompt.lower() == '[stop]') and (break_prompt.lower() != 'y' or break_prompt.lower() != 'n'):
			...
			for weapon in total_weapon_obj:
				...
				def array_wep_kills_stats():
					...
					stats_kills_obj['kills _per_shot'] = int(damage_per_shot)
